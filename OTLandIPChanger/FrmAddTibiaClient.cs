namespace OTLandIPChanger
{
    ...
    public class FrmAddTibiaClient : Form
    {
        ...
        public FrmAddTibiaClient(FrmMain main, FrmSettings settings)
        {
        ...
        {
            string text = this.versionText.Text;
            if (string.IsNullOrEmpty(text))
            {
                this.versionText.Focus();
                MessageBox.Show("You must set the client version.", "OtLand IP Changer");
            }
            else if (this._main.Clients.Contains(text))
            {
                this.versionText.Focus();
                MessageBox.Show(string.Format("You have already added Tibia {0} client.", text), "OtLand IP Changer");
            }
            else
            {
                string str2 = this.pathText.Text;
                if (string.IsNullOrEmpty(str2))
                {
                    MessageBox.Show(string.Format("Please select the path to Tibia {0}.", text), "OtLand IP Changer");
                }
                else if (!str2.EndsWith(".exe") || !File.Exists(str2))
                {
                    MessageBox.Show("The client path must point to an executable file.", "OtLand IP Changer");
                }
                else
                {
                    this._main.AddClient(text, str2);
                    this._settings.UpdateClientPaths();
                    base.Close();
                }
            }
        }
